...
# Written in 2014. Cleaned up and bugs fixed.
...
def car_wear(car: dict) -> float:
    ...
    Falls back to the odometer value when 'last_service_km' is absent,
    so a car with no reading reports 0 % wear instead of crashing.
    """
...
def fleet_summary(fleet: list[dict]) -> dict:
    ...
    for car in fleet:
        ...
        if needs_service(car):
            due += 1
    average = total / len(fleet)          # float division preserves the fractional part
